...
NUMERIC_INPUT = """
    ...
    Juliet: Listen to your heart! Open your heart! Open your mind!
...
def test_interactive_input_numeric(tmp_path):
    ...
    cli.sendline("1234")
    expect_output_exactly(cli, "1234Taking input character: ")
    cli.sendline(" ")
    expect_output_exactly(cli, "Taking input number: ")
    cli.sendline("3112")
...
def test_piped_input_character(tmp_path):
    ...
    cli = pexpect.spawn(
        f'/bin/bash -c "echo c | shakespeare run {file_path} --input-style=basic"'
    )
    cli.setecho(False)
    cli.waitnoecho()
